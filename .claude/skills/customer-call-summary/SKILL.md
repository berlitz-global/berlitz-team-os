...
2. Update template metadata display - currently shows "generated by AI Tutor"
...
4. Add project detail link-outs and environment indicators (dev/staging/prod,
   AI Tutor Teams vs. AI Tutor Pro) to generated project cards
5. Add a "view deployment" button on generated projects so Acme can check
   deploy status without leaving AI Tutor
...
projects that actually make it to production. So I think AI Tutor is sort of
...
templates for all tiers (including AI Tutor Teams) as an additional lever, which
...
review process built around AI Tutor. He's already assigned Priya and Tom
to develop an internal rollout plan before the feature even ships. This validates
our hypothesis that Access Controls and Team Workspaces will expand AI Tutor's user base within
...
Deployment Visibility Within AI Tutor
Priya also flagged the need to surface deployment status within AI Tutor so they
...
deployed through AI Tutor Teams, it's already running on our infrastructure. So we're
not spinning up duplicate instances." We'll solve this by bringing the Deployment
Dashboard into AI Tutor (long-standing ask across multiple AI Tutor customers).
